...
**Row 1 — X is a static tile, on purpose.** The
[Cool-GIFs](https://github.com/Anmol-Baranwal/Cool-GIFs-For-GitHub) set predates the
rebrand and only carries the old Twitter bird, and there is no animated X anywhere in
it. Rather than ship outdated branding, `assets/x-logo.png` is generated locally — black
rounded tile, white mark, matching X's actual identity. LinkedIn and Discord stay
animated.

**Row 2 — the terminal chips.** `assets/chip-*.png`, generated by
`assets/make-chips.py`. Each is drawn at 1:1 in
[Press Start 2P](https://fonts.google.com/specimen/Press+Start+2P) and upscaled with
`NEAREST`, which is what keeps the pixels hard-edged.

> **Never set `width=` or `height=` on these.** They must render at their natural size.
> Any non-integer scaling makes the browser resample them, and the blur it introduces
> defeats the entire reason for drawing them this way. They're built at 2× (40px tall)
> precisely so they sit at a sensible badge height without needing to be resized.

They replaced shields.io badges because shields cannot do a pixel font, has **no CodePen
icon at all**, and renders the Gmail mark in its brand red rather than white. The chips
solve all three: hand-drawn white glyphs — envelope, globe, CodePen's hexagon, a Discord
blob — at whatever colour the palette wants.

To change one, edit the `CHIPS` list in `make-chips.py` and re-run it:

```bash
cd assets && python make-chips.py
```

It fetches the font automatically on first run (`assets/PressStart2P.ttf`, gitignored so
no font binary lands in the repo).

The Discord chip is deliberately **not a link**. Discord usernames aren't
URL-addressable — only numeric IDs are — so `discord.com/users/Naman1201` goes nowhere
useful. The username shows on hover instead.
